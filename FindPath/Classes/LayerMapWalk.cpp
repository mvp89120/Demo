	...
	case LayerMapWalk::OperationMode::RoleWalk:
		{
			...
			if (pMwv)
			{
				...
				//const auto& PathTree = Dijkstra.GetResult().PathTree;
				MwvSeq.push_back(pMwv);
				//for (auto it = PathTree.find(pVertex), end = PathTree.end(); it->second != 0 && it != end; it = PathTree.find(it->second)) // find方法传入的是key
				//{
				//	MwvSeq.push_back((MapWalkVertex*)it->second->UserData["mwv"]);
				//}
				for (Vertex *pParent = pVertex->PathfindingData.pParent; pParent != 0; pParent = pParent->PathfindingData.pParent)
				{
...
void LayerMapWalk::AddEdge(MapWalkVertex* pMwv1, MapWalkVertex* pMwv2)
{
	...
	if (eo1.find(pGv2->GetId()) != eo1.end() || eo2.find(pGv1->GetId()) != eo2.end())
	{
		return;
	}
	...
	// 添加一条边
	m_pGraph->AddEdge(pMwv1->GetGraphVertex()->GetId(), pMwv2->GetGraphVertex()->GetId(), Distance);
	// 因为是有向边，所以，反向也添加一条边
	m_pGraph->AddEdge(pMwv2->GetGraphVertex()->GetId(), pMwv1->GetGraphVertex()->GetId(), Distance);
